...
export function ProductAnalyticsModals({
 ...
 products.forEach(p => {
 if (p.variantsEnabled && p.inventorySkus && p.inventorySkus.length > 0) {
 p.inventorySkus.forEach(sku => {
 const key = sku.skuId;
 const labelParts = [sku.color, sku.size, sku.material].filter(Boolean);
 const label = sku.skuCode || (labelParts.length > 0 ? labelParts.join("") :"Custom SKU");
 variantSalesMap[key] = {
 productId: p.id,
 productName: p.name,
 color: label,
 ...
 } else if (p.variantsEnabled && p.colorVariants && p.colorVariants.length > 0) {
...
export function AdvancedAnalyticsDashboard({
 ...
 let currentId: string | null | undefined = item.productId;
 while (currentId) {
 ...
 const currProd = products.find(p => p.id === currentId);
 currentId = currProd?.parentId;
 }
 });
 ...
 const nType = p.nodeType ||"Category"; // fallback
 if (nType ==="Category" && stats.unitsSold > topCategory.units) topCategory = { name: p.name, units: stats.unitsSold };
 if (nType ==="Product Family" && stats.unitsSold > topFamily.units) topFamily = { name: p.name, units: stats.unitsSold };
 if (nType ==="Model" && stats.unitsSold > topModel.units) topModel = { name: p.name, units: stats.unitsSold };
 if ((nType ==="SKU" || nType ==="Sellable SKU" || p.isLeaf) && stats.unitsSold > topSkuUnits.units) topSkuUnits = { name: p.name, units: stats.unitsSold };
 if ((nType ==="SKU" || nType ==="Sellable SKU" || p.isLeaf) && stats.revenueGenerated > topSkuRev.rev) topSkuRev = { name: p.name, rev: stats.revenueGenerated };
 });
