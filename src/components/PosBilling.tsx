...
import { Product, Customer, Invoice, InvoiceItem, CompanySettings, InvoiceStatus, Agent, InventorySKU, SimpleVariant, ProductSize } from"../types";
...
export default function PosBilling({
 ...
 const treeCats = products.filter(p => p.isLeaf === false);
 if (treeCats.length > 0) {
 const roots = treeCats.filter(c => !c.parentId || !products.some(p => p.id === c.parentId));
 ...
 const updateRowProduct = (index: number, prodId: string) => {
 const updated = [...lineItems];
 const product = products.find((p) => p.id === prodId);
 updated[index].productId = prodId;
 updated[index].customPrice = product ? product.price : 0;
 updated[index].searchQuery = product ? product.name :"";
 updated[index].displayName = product ? product.name :"";
 updated[index].isDropdownOpen = false;
 updated[index].selectedColor = product?.inventoryType !=="Made To Order" && product?.inventoryType !=="Service" && product?.variantsEnabled && product?.colorVariants && product.colorVariants.length > 0
 ? product.colorVariants[0].color
 : undefined;
 updated[index].skuId = undefined; // unset SKUs for legacy product selections
 
 setLineItems(mergeDuplicateLineItems(updated));
 };

 const updateRowProductSku = (index: number, product: Product, sku: InventorySKU) => {
 const updated = [...lineItems];
 updated[index].productId = product.id;
 updated[index].hierarchyNodeId = product.id;
 updated[index].skuId = sku.skuId;
 updated[index].skuCode = sku.skuCode;
 updated[index].customPrice = sku.price;
 const hierarchy = product.name;
 
 const optionLabels = (sku as any).attributes ? (sku as any).attributes.map((a: any) => a.value) : [sku.color, sku.size, sku.material].filter(Boolean);
 
 const isPrefixed = sku.skuCode && sku.skuCode.toLowerCase().startsWith(product.name.toLowerCase());
 const baseName = isPrefixed ? sku.skuCode : [hierarchy, sku.skuCode !== product.name ? sku.skuCode : null].filter(Boolean).join(" - ");
 const pathComplete = [baseName, ...optionLabels].filter(Boolean).join(" ");
 
 updated[index].hierarchyPath = pathComplete;
 updated[index].searchQuery = pathComplete;
 updated[index].displayName = pathComplete;
 updated[index].isDropdownOpen = false;
 updated[index].selectedColor = undefined;
 
 if ((sku as any).attributes) {
 const opts: Record<string, string> = {};
 (sku as any).attributes.forEach((a: any) => opts[a.name] = a.value);
 updated[index].selectedOptions = opts;
 }
 
 setLineItems(updated); // Do not merge duplicate line items when dealing with explicit SKUs for now
 };
 ...
 finalInvoiceNo = resJson.invoiceNumber;
 ...
 rowHsn ="";
 if (prod) {
 if (prod.nodeType ==="SKU") {
 rowHsn = prod.hsnCode ||"";
 } else if (prod.inventorySkus && row.skuId) {
 const matchedSku = prod.inventorySkus.find(s => s.skuId === row.skuId);
 if (matchedSku) {
 rowHsn = matchedSku.hsnCode ||"";
 }
 } else {
 rowHsn = prod.hsnCode ||"";
 }
 ...
 const matchedProduct = updatedProducts.find(p => p.id === item.productId);
 if (matchedProduct) {
...
"None",
 ...
 {item.productId && (() => {
 const hsn = selectedProd?.nodeType ==="SKU"
 ? selectedProd.hsnCode
 : (selectedProd?.inventorySkus && item.skuId)
 ? selectedProd.inventorySkus.find(s => s.skuId === item.skuId)?.hsnCode
 : undefined;
 return (
 ...
 type="text"
 placeholder="9403"
 value={item.hsnCode !== undefined ? item.hsnCode : (() => {
 const masterHsn = selectedProd?.nodeType ==="SKU"
 ? selectedProd.hsnCode
 : (selectedProd?.inventorySkus && item.skuId)
 ? selectedProd.inventorySkus.find(s => s.skuId === item.skuId)?.hsnCode
 : selectedProd?.hsnCode ||"9403";
 return masterHsn ||"9403";
 })()}
 onChange={(e) => {
 const updated = [...lineItems];
 updated[index] = {
