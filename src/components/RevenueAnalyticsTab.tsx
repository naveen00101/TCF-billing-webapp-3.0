...
export default function RevenueAnalyticsTab({ 
 ...
 const trendsAndBreakdowns = useMemo(() => {
    // Pre-group invoice items by normalized invoiceNo and invoiceId for O(N + M) speed
    const itemsByInvoiceNo = new Map<string, InvoiceItem[]>();
    const itemsByInvoiceId = new Map<string, InvoiceItem[]>();
    fallbackInvoiceItems.forEach(item => {
      const itemNo = String(item.invoiceNo || "").trim().toLowerCase();
      const itemId = String(item.invoiceId || "").trim().toLowerCase();
      if (itemNo) {
        if (!itemsByInvoiceNo.has(itemNo)) itemsByInvoiceNo.set(itemNo, []);
        itemsByInvoiceNo.get(itemNo)!.push(item);
      }
      if (itemId) {
        if (!itemsByInvoiceId.has(itemId)) itemsByInvoiceId.set(itemId, []);
        itemsByInvoiceId.get(itemId)!.push(item);
      }
    });
...
"No GST": { name:"Exempted / Plain", cgst: 0, sgst: 0, igst: 0, total: 0 },
 ...
 gstMap[key].total += getInvoiceGst(inv);
  ...
  const invNo = String(inv.invoiceNo || "").trim().toLowerCase();
  const invId = String(inv.invoiceId || "").trim().toLowerCase();
  const items = (invNo ? itemsByInvoiceNo.get(invNo) : null) || (invId ? itemsByInvoiceId.get(invId) : null) || [];
 items.forEach(item => {
 // Product
 ...
 // Try to locate parent reference from simple variants
 if (!pDet && item.skuId) {
 pDet = fallbackProducts.find(p => p.simpleVariants?.some(s => s.id === item.skuId));
 }
 if (!pDet && pId) {
 pDet = fallbackProducts.find(p => p.simpleVariants?.some(s => s.id === pId));
 }

 let resolvedCategory = "Unknown";
 if (pDet && pDet.category && pDet.category.trim() !== "" && pDet.category.trim().toLowerCase() !== "unknown") {
   resolvedCategory = pDet.category.trim();
 }
