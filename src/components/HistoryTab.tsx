...
"Address Updated",
 ...
 const soldItemsForThisNode = activeItems.filter(i => p.nodeType ==="SKU" ? i.skuId === p.id : i.productId === p.id);
 ...
 // Multiplier: cancellation restores stock (+qty) and removes revenue (-rev).
 // Un-cancelling/restoring reduces stock (-qty) and adds revenue (+rev).
 const qtyMultiplier = isCancellation ? 1 : -1;
 const revMultiplier = isCancellation ? -1 : 1;

 if (p.nodeType ==="SKU") {
 const oldStock = p.stockAvailable ?? 0;
 const finalSkuStock = Math.max(oldStock + (totalQtySold * qtyMultiplier), 0);
 return {
 ...p,
 stockAvailable: finalSkuStock,
 ...
 // Deduct/restore SKU level stock within parent's inventorySkus
 let updatedSkus = p.inventorySkus ? [...p.inventorySkus] : undefined;
 if (updatedSkus && updatedSkus.length > 0) {
 updatedSkus = updatedSkus.map(sku => {
 const matchingSoldItem = soldItemsForThisNode.find(i => i.skuId === sku.skuId);
 if (matchingSoldItem) {
 const oldSkuStock = sku.stock ?? 0;
 const finalSkuStock = Math.max(oldSkuStock + (matchingSoldItem.quantity * qtyMultiplier), 0);
 return {
 ...sku,
 stock: finalSkuStock,
 unitsSold: Math.max((sku.unitsSold || 0) + (matchingSoldItem.quantity * revMultiplier), 0),
 revenueGenerated: Math.max((sku.revenueGenerated || 0) + (matchingSoldItem.amount * revMultiplier), 0)
 };
 }
 return sku;
 });
 ...
 // Deduct/restore variant level stock (Legacy fallback)
 let updatedVariants = p.colorVariants ? [...p.colorVariants] : undefined;
 if (p.variantsEnabled && updatedVariants && updatedVariants.length > 0) {
 updatedVariants = updatedVariants.map(v => {
 const matchingSoldItem = soldItemsForThisNode.find(i => (i.selectedColor ||"").toLowerCase() === v.color.toLowerCase());
 if (matchingSoldItem) {
 return {
 ...v,
 stock: Math.max(v.stock + (matchingSoldItem.quantity * qtyMultiplier), 0),
 unitsSold: Math.max((v.unitsSold || 0) + (matchingSoldItem.quantity * revMultiplier), 0),
 revenueGenerated: Math.max((v.revenueGenerated || 0) + (matchingSoldItem.amount * revMultiplier), 0)
 };
 }
 return v;
 });
 ...
 let finalOverallStock = p.stockAvailable;
 const isStockItem = p.inventoryType === 'Stock Item' || !p.inventoryType;
 if (isStockItem) {
 if (updatedSkus && updatedSkus.length > 0) {
 finalOverallStock = updatedSkus.reduce((sum, s) => sum + (s.stock ?? 0), 0);
 } else if (p.variantsEnabled && updatedVariants && updatedVariants.length > 0) {
 finalOverallStock = updatedVariants.reduce((sum, v) => sum + v.stock, 0);
 } else {
 finalOverallStock = Math.max((p.stockAvailable || 0) + (totalQtySold * qtyMultiplier), 0);
 }
 }
 ...
 stockAvailable: finalOverallStock
 };
 }
 }
 return p;
