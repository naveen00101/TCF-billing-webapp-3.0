...
// Memory cache for the resolved logo base64 and aspect ratio to prevent repeated fetches and canvas operations
let cachedLogoBase64: string | null = null;
let cachedLogoAspect = 1.33;
...
export async function preloadPDFLogo(): Promise<void> {
  ...
  try {
        ...
        if (base64) {
          ...
          if (img.width && img.height) {
            ...
            if (ctx) {
              ...
              cachedLogoBase64 = canvas.toDataURL("image/jpeg", 0.8);
              cachedLogoAspect = aspect;
              console.log("[PDF Generator] Logo preloaded and cached successfully.");
            }
          }
        }
      }
    ...
    console.warn("[PDF Generator] Failed to preload PDF logo:", e);
  }
}
...
export async function generateInvoicePDF(
  ...
  // Load logo image as base64 dynamically (or retrieve from cache)
  let logoBase64: string | null = cachedLogoBase64;
  let logoAspect = cachedLogoAspect;
  ...
  if (!logoBase64) {
    try {
      ...
      // Update cache for subsequent generations
      if (logoBase64) {
        cachedLogoBase64 = logoBase64;
        cachedLogoAspect = logoAspect;
      }
    } catch (e) {
