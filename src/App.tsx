...
export default function App() {
 ...
 // Mount loading
  useEffect(() => {
     async function bootSequence() {
       ...
       // 0. Preload database cache from IndexedDB
       await SheetsSyncEngine.preloadCache();
       
       // Minimum loader display of 600ms for visual transition smoothness
 ...
 reloadApplicationState();
  ...
  // Render loading screen if database has not finished preloading
  if (!isDbLoaded) {
    return (
      <div className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-blue-900/20 via-surface to-surface flex flex-col justify-center items-center font-sans relative overflow-hidden">
        {/* Subtle grid pattern background */}
        <div className="absolute inset-0 bg-[linear-gradient(to_right,#8080800a_1px,transparent_1px),linear-gradient(to_bottom,#8080800a_1px,transparent_1px)] bg-[size:14px_24px] pointer-events-none"></div>
        
        {/* Ambient background glow */}
        <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-blue-500/10 blur-[128px] rounded-full pointer-events-none animate-pulse"></div>

        <div className="relative flex flex-col items-center gap-6 p-8 rounded-3xl bg-card/60 backdrop-blur-xl border border-default/50 shadow-2xl max-w-sm w-full text-center">
          {/* Animated Database Icon */}
          <div className="relative">
            <div className="absolute -inset-1.5 rounded-full bg-gradient-to-tr from-blue-600 to-indigo-600 blur opacity-40 animate-pulse"></div>
            <div className="relative bg-gradient-to-tr from-blue-600 to-indigo-600 p-4 rounded-2xl shadow-lg shadow-blue-500/20 text-white">
              <Database className="h-8 w-8 animate-bounce" />
            </div>
          </div>

          <div className="flex flex-col gap-2">
            <h2 className="text-xl font-display font-semibold tracking-tight text-primary">
              Connecting Database
            </h2>
            <p className="text-xs text-muted max-w-[240px] leading-relaxed">
              Preloading tables and establishing sync tunnels with offline IndexedDB storage.
            </p>
          </div>

          {/* Premium custom loading bar */}
          <div className="w-full h-1 bg-default/40 rounded-full overflow-hidden relative">
            <div className="absolute top-0 bottom-0 left-0 bg-gradient-to-r from-blue-600 via-indigo-500 to-blue-600 w-1/2 rounded-full animate-[loading_1.5s_infinite_ease-in-out]"></div>
          </div>
        </div>

        {/* Global style tag for the custom loading bar animation */}
